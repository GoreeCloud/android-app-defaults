...
Current `main` includes the first persistent user-facing path plus the next M1 editor slice: tracker type choice, validated persistent creation, user-selected past local date/time with explicit IANA zone input, populated Dashboard cards, persisted Tracker Details, Edit Tracker/current-period start updates, lifecycle-aware minute updates, and deliberate persisted display-format changes. Icon/accent selection remains open because no approved Since-specific GLAZE key mapping is verified. The active M1 accessibility candidate adds coherent tracker-card TalkBack semantics, full-row display-format radio targets, shared Dashboard minute ticking, and Compose instrumentation coverage for card semantics plus custom-start/create/edit behavior. Reset/history, goal progress/editor, archive/search/settings, portability/recovery, platform-system acceptance, and release gates remain open.
